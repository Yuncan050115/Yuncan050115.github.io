...
date: 2023-07-09 17:51:12
updated: 2023-07-09 17:51:12
---

![原图遗失，AI生成](content/assets/ai-2023-07-09-排序算法-数据结构与算法-五--5.jpg)
...
![image-20220904212453328](https://oss.itbaima.cn/internal/markdown/2022/09/04/BYOvgd3XCspNI9i.png)

那么在第一轮排序时，首先比较前两个元素：

![image-20220904212608834](https://oss.itbaima.cn/internal/markdown/2022/09/04/VuiIDqPAr6SMd7H.png)

我们发现前者更大，那么此时就需要交换，交换之后，继续向后比较后面的两个元素：

![image-20220904212637156](https://oss.itbaima.cn/internal/markdown/2022/09/04/CxObP3Tkm4uzU98.png)

我们发现后者更大，不变，继续看后两个：

![image-20220904212720898](https://oss.itbaima.cn/internal/markdown/2022/09/04/jy5PtuvHO2BTQc8.png)

此时前者更大，交换，继续比较后续元素：

![image-20220904212855292](https://oss.itbaima.cn/internal/markdown/2022/09/04/64qSbtyMXQ37BDk.png)

还是后者更大，继续交换，然后向后比较：

![image-20220904212942212](https://oss.itbaima.cn/internal/markdown/2022/09/04/Rq1xwzupm8C3Q2Z.png)

依然是后者更大，我们发现，只要是最大的元素，它会在每次比较中被一直往后丢：

![image-20220904213034375](https://oss.itbaima.cn/internal/markdown/2022/09/04/vx16PpyFkKhVzsJ.png)

最后，当前数组中最大的元素就被丢到最前面去了，这一轮排序结束，因为最大的已经排到对应的位置上了，所以说第二轮我们只需要考虑其之前的这些元素即可：

![image-20220904213115671](https://oss.itbaima.cn/internal/markdown/2022/09/04/qWjTBSe4rlbUxIf.png)
...
![image-20220904214541199](https://oss.itbaima.cn/internal/markdown/2022/09/04/RuSePqkc4ydVCHt.png)
...
![image-20220904221510897](https://oss.itbaima.cn/internal/markdown/2022/09/04/Pd24brBwliqZuLh.png)

将其取出，从后往前，与前面的有序序列依次进行比较，首先比较的是4，发现比4小，继续向前，发现已经到头了，所以说直接放到最前面即可，注意在放到最前面之前，先将后续元素后移，腾出空间：

![image-20220904221648492](https://oss.itbaima.cn/internal/markdown/2022/09/04/JWbfZ9mVyrRMIvX.png)

接着插入即可：

![image-20220904221904359](https://oss.itbaima.cn/internal/markdown/2022/09/04/ZkhRcGqr8Ays93z.png)

目前前面两个元素都是有序的状态了，我们继续来看第三个元素：

![image-20220904221938583](https://oss.itbaima.cn/internal/markdown/2022/09/04/i2o4REcPXBaTWpg.png)

 依然是从后往前看，我们发现上来就遇到了7小的4，所以说直接放到这个位置：

![image-20220904222022949](https://oss.itbaima.cn/internal/markdown/2022/09/04/vZ4rXTmCsx5ikdJ.png)

现在前面三个元素都是有序状态了，同样的，我们继续来看第四个元素：

![image-20220904222105375](https://oss.itbaima.cn/internal/markdown/2022/09/04/YLheUfDB8HnuEk1.png)

依次向前比较，发现到头了都没找到比1还小的元素，所以说将前面三个元素全部后移：

![image-20220904222145903](https://oss.itbaima.cn/internal/markdown/2022/09/04/7pz8NDEZbmTXaJY.png)

将1插入到对应的位置上去：

![image-20220904222207544](https://oss.itbaima.cn/internal/markdown/2022/09/04/6bnyKs3Iq7S5cYC.png)
...
![image-20220905141347927](https://oss.itbaima.cn/internal/markdown/2022/09/05/JAoXIHwBDrg3y8n.png)

交换之后，第一个元素已经是有序状态了，我们继续从剩下的元素中寻找一个最小的：

![image-20220905141426011](https://oss.itbaima.cn/internal/markdown/2022/09/05/DsKk5GP6RJTOCXh.png)

此时2正好在第二个位置，假装交换一下，这样前面两个元素都已经是有序的状态了，我们接着来看剩余的：

![image-20220905141527050](https://oss.itbaima.cn/internal/markdown/2022/09/05/3B4n9cFKdxvWXtQ.png)

此时发现3是最小的，所以说直接将其交换到第三个元素位置上：

![image-20220905141629207](https://oss.itbaima.cn/internal/markdown/2022/09/05/iTt8UoAPRHIG9lJ.png)
...
![image-20220905210056432](https://oss.itbaima.cn/internal/markdown/2022/09/05/LneNjifuZ4JOgKl.png)

此时从右往左开始看，直到遇到一个比4小的元素，首先是6，肯定不是，将指针往后移动：

![image-20220905210625181](https://oss.itbaima.cn/internal/markdown/2022/09/05/zVkd4wvAq9FK2ET.png)

此时继续让3和4进行比较，发现比4小，那么此时直接将3交换（其实直接覆盖过去就行了）到左边指针所指向的元素位置：

![image-20220905210730105](https://oss.itbaima.cn/internal/markdown/2022/09/05/Z7y3sO5qDWxYdiL.png)

此时我们转为从左往右看，如果遇到比4大的元素，就交换到右边指针处，3肯定不是了，因为刚刚才缓过来，接着就是2：

![image-20220905210851474](https://oss.itbaima.cn/internal/markdown/2022/09/05/cXk7xEp2IfSH9iq.png)

2也没有4大，所以说继续往后看，此时7比4要大，那么继续交换：

![image-20220905211300102](https://oss.itbaima.cn/internal/markdown/2022/09/05/zn8asNlCbOyv1cA.png)

接着，又开始从右往左看：

![image-20220905211344027](https://oss.itbaima.cn/internal/markdown/2022/09/05/APvaMt4jJV79Dpn.png)

此时5是比4要大的，继续向前，发现1比4要小，所以说继续交换：

![image-20220905211427939](https://oss.itbaima.cn/internal/markdown/2022/09/05/cCi1xkSfIVjEsPK.png)

接着又转为从左往右看，此时两个指针撞到一起了，排序结束，最后两个指针所指向的位置就是给基准元素的位置了：

![image-20220905211543845](https://oss.itbaima.cn/internal/markdown/2022/09/05/eaRN5ZvVs8bc9lU.png)

本轮快速排序结束后，左边不一定都是有序的，但是一定比基准元素要小，右边一定比基准元素大。接着我们以基准为中心，分成两个部分再次进行快速排序：

![image-20220905211741787](https://oss.itbaima.cn/internal/markdown/2022/09/05/4MLoPCKs1W9Y3Ox.png)
...
![image-20220906131959909](https://oss.itbaima.cn/internal/markdown/2022/09/06/aEbUInwHOTi1GF7.png)

整个数组正好是倒序的，那么相当于上来就要把整个数组找完，然后把8放到最后一个位置，此时第一轮结束：

![image-20220906132112592](https://oss.itbaima.cn/internal/markdown/2022/09/06/erMHzcW58vVqFGa.png)

由于8直接跑到最右边了，那么此时没有右半部分，只有做半部分，此时左半部分继续进行快速排序：

![image-20220906132244369](https://oss.itbaima.cn/internal/markdown/2022/09/06/6yQa7e8VTYgpUZN.png)

此时1又是最小的一个元素，导致最后遍历完了，1都还是在那个位置，此时没有左半部分，只有右半部分：

![image-20220906132344525](https://oss.itbaima.cn/internal/markdown/2022/09/06/r9LlRfEotZMmdDn.png)

此时基准是7，又是最大的，真是太倒霉了，排完之后7跑到最左边，还是没有右半部分：

![image-20220906132437765](https://oss.itbaima.cn/internal/markdown/2022/09/06/PicWXjIBMnfUd7H.png)
...
![image-20220906132945691](https://oss.itbaima.cn/internal/markdown/2022/09/06/2YBc1goqMGwuTs4.png)

此时第一个元素和最后一个元素都作为基准元素，将整个返回划分为三段，假设基准1小于基准2，那么第一段存放的元素全部要小于基准1，第二段存放的元素全部要不小于基准1同时不大于基准2，第三段存放的元素全部要大于基准2：

![image-20220906133219853](https://oss.itbaima.cn/internal/markdown/2022/09/06/MsvJC1OtnbuGye9.png)

因此，在划分为三段之后，每轮双轴快排结束后需要对这三段分别继续进行双轴快速排序，最后就可以使得整个数组有序了，当然这种排序算法更适用于哪些量比较大的数组，如果量比较小的话，考虑到双轴快排要执行这么多操作，其实还不如插入排序来的快。

我们来模拟一下双轴快速排序是如何进行的:

![image-20220906140255444](https://oss.itbaima.cn/internal/markdown/2022/09/06/WFASGVCJaQHhBX3.png)

首先取出首元素和尾元素作为两个基准，然后我们需要对其进行比较，如果基准1大于基准2，那么需要先交换两个基准，只不过这里因为4小于6，所以说不需要进行交换。

此时我们需要创建三个指针：

![image-20220906140538076](https://oss.itbaima.cn/internal/markdown/2022/09/06/y283Ne7M6XmUtZA.png)
...
![image-20220906141556398](https://oss.itbaima.cn/internal/markdown/2022/09/06/HA8tKzv6Uri45p1.png)

同样的，我们继续来看橙色指针所指元素，此时为7，大于基准2，那么此时需要在右边找到一个不大于基准2的元素：

![image-20220906141653453](https://oss.itbaima.cn/internal/markdown/2022/09/06/Nj2PvlyYCnSb3kV.png)

绿色指针从右开始向左找，此时找到3，直接交换橙色指针和蓝色指针元素：

![image-20220906141758610](https://oss.itbaima.cn/internal/markdown/2022/09/06/J3ymMprRPTSHFWi.png)

下一轮开始继续看橙色指针元素，此时发现是小于基准1的，所以说先向前移动蓝色指针，发现和橙色又在一起了，交换了跟没交换一样，此时两个指针都后移了一位：

![image-20220906141926006](https://oss.itbaima.cn/internal/markdown/2022/09/06/7iSnb8ctryz6A1l.png)

新的一轮继续来看橙色指针所指元素，此时我们发现1也是小于基准1的，先移动蓝色指针，再交换，在移动橙色指针，跟上面一样，交换个寂寞：

![image-20220906142041202](https://oss.itbaima.cn/internal/markdown/2022/09/06/wrgQAsSTHjd5Ynq.png)

此时橙色指针指向8，大于基准2，那么同样需要在右边继续找一个不大于基准2的进行交换：

![image-20220906142134949](https://oss.itbaima.cn/internal/markdown/2022/09/06/udGnpJHzxP8SK4l.png)

此时找到5，满足条件，交换即可：

![image-20220906142205055](https://oss.itbaima.cn/internal/markdown/2022/09/06/oEycNqQVhbuC3Tx.png)

我们继续来看橙色指针，发现此时橙色指针元素不小于基准1且不大于基准2，那么根据前面的规则，只需要向前移动橙色指针即可：

![image-20220906142303329](https://oss.itbaima.cn/internal/markdown/2022/09/06/YSxmL2tiyBQGl8C.png)

此时橙色指针和绿色指针撞一起了，没有剩余待排序元素了，最后我们将两个位于两端点基准元素与对应的指针进行交换，基准1与蓝色指针交换，基准2与绿色指针进行交换：

![image-20220906142445417](https://oss.itbaima.cn/internal/markdown/2022/09/06/C2AnBDiUp7qxuSb.png)
...
![image-20220905223505975](https://oss.itbaima.cn/internal/markdown/2022/09/05/GLEjym78BWNUtCf.png)

首先数组长度为8，直接整除2，得到34，那么步长就是4了，我们按照4的步长进行分组：

![image-20220905223609936](https://oss.itbaima.cn/internal/markdown/2022/09/05/S72NcgKBzIA54om.png)

其中，4、8为第一组，2、5 为第二组，7、3为第三组，1、6为第四组，我们分别在这四组内进行插入排序，组内排序之后的结果为：

![image-20220905223659584](https://oss.itbaima.cn/internal/markdown/2022/09/05/XO4a9HKP7vhGTmf.png)

可以看到目前小的元素尽可能地在往前面走，虽然还不是有序的，接着我们缩小步长，4/2=2，此时按照这个步长划分：

![image-20220905223804907](https://oss.itbaima.cn/internal/markdown/2022/09/05/sdKhcGmapgOZkCP.png)

此时4、3、8、7为一组，2、1、5、6为一组，我们继续在这两个组内进行排序，得到：

![image-20220905224111803](https://oss.itbaima.cn/internal/markdown/2022/09/05/MD8UFxJvg274Qw6.png)
...
![image-20220906001134488](https://oss.itbaima.cn/internal/markdown/2022/09/06/iedURC8vVpcMgn6.png)
...
![image-20220906220020172](https://oss.itbaima.cn/internal/markdown/2022/09/06/GTkWlzpZbxMX58K.png)
...
![image-20220906221306519](https://oss.itbaima.cn/internal/markdown/2022/09/06/bpnSDlOLKawGRxT.png)

接着我们来看倒数第二个非叶子结点，也就是7，那么此时两个孩子都是小于它的，所以说不需要做任何调整，我们接着来看倒数第三个非叶子结点2，此时2的两个孩子6、8都大于2，那么我们选择两个孩子里面一个最大的交换上去：

![image-20220906221504364](https://oss.itbaima.cn/internal/markdown/2022/09/06/svNPURhJbzAQo9d.png)
...
![image-20220906221833012](https://oss.itbaima.cn/internal/markdown/2022/09/06/MmzbFTLYSZAW4gv.png)

交换之后，此时整个二叉树就满足大顶堆的性质了，我们第一次初始调整也就完成了。

此时开始第二步，我们需要一个一个地将堆顶元素往后面进行交换，相当于每次都去取一个最大的出来，直到取完，首先交换堆顶元素和最后一个元素：

![image-20220906222327297](https://oss.itbaima.cn/internal/markdown/2022/09/06/8fBeubxnqD1rJEZ.png)

此时整个数组中最大的元素已经排到对应的位置上了，然后我们不再考虑最后一个元素，此时将前面的剩余元素继续看做一棵完全二叉树，对根结点重新进行一次堆化（只需要调整根结点即可，因为其他非叶子结点的没有变动），使得其继续满足大顶堆的性质：

![image-20220906222819554](https://oss.itbaima.cn/internal/markdown/2022/09/06/2RTG76ejsKyCthU.png)

还没完，继续调整：

![image-20220906222858752](https://oss.itbaima.cn/internal/markdown/2022/09/06/clEdF5YBO7iKDP6.png)

此时第一轮结束，接着第二轮，重复上述操作，首先依然是将堆顶元素丢到倒数第二个位置上，相当于将倒数第二大的元素放到对应的位置上去：

![image-20220906222934602](https://oss.itbaima.cn/internal/markdown/2022/09/06/KaZGOsWD2chNf3i.png)

此时已经有两个元素排好序了，同样的，我们继续将剩余元素看做一个完全二叉树，继续对根结点进行堆化操作，使得其继续满足大顶堆性质：

![image-20220906223110734](https://oss.itbaima.cn/internal/markdown/2022/09/06/PD7FBrlqc1R8LWE.png)

第三轮同样的思路，将最大的交换到后面去：

![image-20220906223326135](https://oss.itbaima.cn/internal/markdown/2022/09/06/tKhYbOqv6Ezmc8R.png)
...
![原图遗失，AI生成](content/assets/ai-2023-07-09-排序算法-数据结构与算法-五--6.jpg)
...
![image-20220906232451040](https://oss.itbaima.cn/internal/markdown/2022/09/06/PuBRFqHKDvxIpwZ.png)
...
![image-20220907135544173](https://oss.itbaima.cn/internal/markdown/2022/09/07/oi74M9zRdktHNjC.png)

继续进行划分：

![image-20220907135744253](https://oss.itbaima.cn/internal/markdown/2022/09/07/yFe9d7XDrqH8MWJ.png)

最后会变成这样的一个一个的元素：

![image-20220907135927289](https://oss.itbaima.cn/internal/markdown/2022/09/07/ZdV5RWpAjQzeHn1.png)

此时我们就可以开始归并排序了，注意这里的合并并不是简简单单地合并，我们需要按照从小到大的顺序，依次对每个元素进行合并，第一组树4和2，此时我们需要从这两个数组中先选择小的排到前面去:

![image-20220907140219455](https://oss.itbaima.cn/internal/markdown/2022/09/07/Is1lUMzTaSdk7bB.png)

排序完成后，我们继续向上合并：

![image-20220907141217008](https://oss.itbaima.cn/internal/markdown/2022/09/07/eAsX37G2BKdNHZj.png)

最后我们再将这两个数组合并到原有的规模：

![image-20220907141442229](https://oss.itbaima.cn/internal/markdown/2022/09/07/rq7lDhdvTQympUS.png)
...
![image-20220907142933725](https://oss.itbaima.cn/internal/markdown/2022/09/07/5H29S1YpwUXh63L.png)

我们先对其进行一次遍历，统计每个元素的出现次数，统计完成之后，我们就能够明确在排序之后哪个位置可以存放值为多少的元素了：

![image-20220907145336855](https://oss.itbaima.cn/internal/markdown/2022/09/07/dBvf8LYJnl47HmN.png)

我们来分析一下，首先1只有一个，那么只会占用一个位置，2也只有一个，所以说也只会占用一个位置，以此类推：

![image-20220907145437992](https://oss.itbaima.cn/internal/markdown/2022/09/07/FXOMDrohvgNECLj.png)

所以说我们直接根据统计的结果，把这些值挨个填进去就行了，而且还是稳定的，按顺序，有几个填几个就可以了：

![image-20220907145649061](https://oss.itbaima.cn/internal/markdown/2022/09/07/R4rPLtDVxYZcvEI.png)
...
![image-20220907143715938](https://oss.itbaima.cn/internal/markdown/2022/09/07/ImGw7Z6xhBnLAWr.png)

这样，我们只需要遍历一次，就可以将所有的元素分类丢到这些桶中，最后我们只需要依次遍历这些桶，然后把里面的元素拿出来依次存放回去得到的就是有序的数组了：

![image-20220907144255326](https://oss.itbaima.cn/internal/markdown/2022/09/07/d4ZMjNS8UmaCRef.png)
...
![image-20220907152403435](https://oss.itbaima.cn/internal/markdown/2022/09/07/c2qtbQJOW7mS6Hr.png)

先按照个位数进行统计，然后排序，再按照十位进行统计，然后排序，最后得到的结果就是最终的结果了：

![image-20220907152903020](https://oss.itbaima.cn/internal/markdown/2022/09/07/WJjtbBMKIVYUlNs.png)

然后是十位数：

![image-20220907153005797](https://oss.itbaima.cn/internal/markdown/2022/09/07/mI4yjLhXSP17cFq.png)

最后再次按顺序取出来：
![image-20220907153139536](https://oss.itbaima.cn/internal/markdown/2022/09/07/2njAIfvDQbLxNyt.png)
...
![image-20220907154254943](https://oss.itbaima.cn/internal/markdown/2022/09/07/oeEcWx91qHp2iXa.png)

我们每次都随机从数组中挑一个元素，与随机的一个元素进行交换：

![image-20220907154428792](https://oss.itbaima.cn/internal/markdown/2022/09/07/PQKGfMTH4xCUNgt.png)
...
![image-20220907160219493](https://oss.itbaima.cn/internal/markdown/2022/09/07/NposWDHJIxuVztO.png)
